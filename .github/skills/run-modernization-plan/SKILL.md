...
7. You MUST update the "Status" and results for each task in modernization-progress.md as follows:
    ...
    - When a task is successfully finished but does not include any code changes, set the Status to "Skipped"
    - When a task is successfully finished with code change, always update BuildResult and UTResult on completion:
        - Set BuildResult to "Success" or "Failed" based on build outcome.
        - Set UTResult to "Success" or "Failed" based on unit test outcome. If the UT error is from infra setup, just ignore it
    - If a task is completed but has Build or Test errors, set Status to "Failed", or set it to "Success"
    - If a task is not started, set the status as Incompleted
